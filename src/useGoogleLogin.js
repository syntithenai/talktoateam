...
export default function useGoogleLogin({clientId, scopes, usePrompt, loginButtonId}) {
    ...
    function initClient(extraScopes) {
      ...
      // console.log("initclient scopes", useScopes)
    ...
    function revokeToken() {
      ...
      setUser(null)
      try {
        window.google.accounts.oauth2.revoke(accessToken.current, () => {console.log('access token revoked')});
      } catch (e) {}
      setAccessToken(null)
    ...
    function handleCredentialResponse(response) {
      // console.log("handle CREDS", response)
      setResponseToken(response.credential)
