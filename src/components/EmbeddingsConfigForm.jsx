    ...
    <Form onSubmit={handleSubmit}>
		...
		<Row className="mb-3">
			<Form.Group as={Row} controlId="openaiKey">
				...
				<Form.Control
					type="text"
					value={config && config.embeddings && config.embeddings.max_length ? config.embeddings.max_length : 511}
			...
			<Form.Label>Use Local Embeddings</Form.Label>
			<FormCheck
			    style={{display:'inline', marginRight:'0.2em', marginLeft:'1em'}} 
				type="checkbox"
				checked={config && ((config.embeddings && config.embeddings.use === 'local') || !config.embeddings || !config.embeddings.use)  ? true : false}
				onChange={(e) => {setConfigValue('use',!(config && config.embeddings && config.embeddings.use === 'local') ? 'local' : ''); setConfigValue('max_length',511)}}
			  />
			...
			<Form.Group as={Col} controlId="openaiKey">
			  <FormCheck
				...
				onChange={(e) => {setConfigValue('use',!(config && config.embeddings && config.embeddings.use === 'openai') ? 'openai' : ''); setConfigValue('max_length',8191)}}
			  />
			  <Form.Label>OpenAI API Key</Form.Label>
			  <Form.Control
				type="text"
				value={config && config.embeddings && config.embeddings.openai_key ? config.embeddings.openai_key : ''}
				onChange={(e) => {setConfigValue('openai_key',e.target.value);  setConfigValue('use','openai'); setConfigValue('max_length',8191)}}
			  />
			  ...
			  <Form.Select value={config && config.embeddings && config.embeddings.openai_model ? config.embeddings.openai_model : ''}
				onChange={(e) => {setConfigValue('openai_model',e.target.value);  setConfigValue('use','openai'); setConfigValue('max_length',8191)}} >
            ...
            <FormCheck
				...
				onChange={(e) => {setConfigValue('use',!(config && config.embeddings && config.embeddings.use === 'self_hosted') ? 'self_hosted' : '') ; setConfigValue('max_length',511)}}
			  />
          ...
          <Form.Control
            ...
            onChange={(e) => {setConfigValue('self_hosted_url',e.target.value); setConfigValue('use','self_hosted'); setConfigValue('max_length',511)}}
          />
          ...
          <Form.Control
            ...
            onChange={(e) => {setConfigValue('self_hosted_key',e.target.value); setConfigValue('use','self_hosted'); setConfigValue('max_length',511)}}
          />
        </Form.Group>
